...
public class ImageCaptureService2 extends Service implements
    ...
    Camera.PictureCallback mCall = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            bmp.compress(Bitmap.CompressFormat.JPEG, 40, bytes1);
            //
            String imagePath = getStorage(getApplicationContext());
           /* File imagesFolder = new File(
                    Environment.getExternalStorageDirectory(), "shoabgee");
            if (!imagesFolder.exists())
                imagesFolder.mkdirs(); // <----*/
            File image = new File(imagePath);

            Log.e("TAg", "the image file path is: " + image);

            SharedPreferences sharedPreferences = getSharedPreferences(SPref.PREF_IMAGS, 0);
            SPref.storingIMAGEFilePathWithKey(sharedPreferences, SPref.IMAGE_PATH2, image.toString());
            ...
            handler.post(new Runnable() {
                ...
                @Override
                public void run() {
                    Log.e("TAG", "Picture has been taken");
                    /*Toast.makeText(getApplicationContext(),
            ...
            stopSelf();
           /* if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                startForegroundService(new Intent(getApplicationContext(), ImageCaptureService3.class));
            } else {
                startService(new Intent(getApplicationContext(), ImageCaptureService3.class));
            }*/
            startService(new Intent(getApplicationContext(), ImageCaptureService3.class));


            PowerButtonBroadCast.counter = 2;
        }
    };
    ...
    private String getStorage(Context context) {
        ...
        if (isSDPresent) {
            fileName = context.getExternalCacheDir().getAbsolutePath() + "/CrighterImages";
        } else {
            fileName = context.getFilesDir() + "/CrighterImages";
        }
        File dir = new File(fileName);
        if (!dir.exists()) {
            dir.mkdirs();
        }
        long timStemp = System.currentTimeMillis();
        fileName=fileName+timStemp+".jpg";
        return fileName;
    }
